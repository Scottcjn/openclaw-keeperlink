...
def test_header_pays_for_its_own_job():
    job = _job("job-0001")
    result = keeperlink_service.verify_x402_payment(_header_for(job), job, _config())
    assert result.valid, result.reason
    assert result.payer.lower() == PAYER.lower()


def test_header_cannot_be_replayed_on_another_job():
    paid = _job("job-0001")
    ...
    result = keeperlink_service.verify_x402_payment(header, stolen, _config())
    assert not result.valid
    assert "not bound to this job" in result.reason


def test_header_cannot_be_reused_after_the_job_is_edited():
    paid = _job("job-0001")
    header = _header_for(paid)

    # Same job_id, bigger swap: the digest moves, so the binding breaks.
    mutated = paid.model_copy(update={"amount_in": "5000"})
    result = keeperlink_service.verify_x402_payment(header, mutated, _config())
    assert not result.valid
    assert "not bound to this job" in result.reason
...
def test_unbound_legacy_nonce_is_rejected():
    job = _job("job-0001")
    ...
    result = keeperlink_service.verify_x402_payment(legacy, job, _config())
    assert not result.valid
...
def test_time_moves_but_binding_holds():
    ...
    assert not keeperlink_service.verify_x402_payment(header, second, _config()).valid
